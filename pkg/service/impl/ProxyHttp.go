...
func (i *ProxyHttp) handleRequest() {
	...
	if err != nil {
		log.Println("获取远程服务器响应失败：" + err.Error())
		return
	}
...
func (i *ProxyHttp) handleSslRequest() {
	...
	if err != nil {
		log.Println("返回连接状态失败：" + err.Error())
		return
	}
	// 建立ssl连接并返回给源
	i.SslReceiveSend()
...
func (i *ProxyHttp) SslReceiveSend() {
	...
	if err != nil {
		log.Println(i.request.Host + "：获取证书失败：" + err.Error())
		return
	}
	...
	if err != nil {
		if err == io.EOF {
			log.Println("浏览器ssl连接断开：" + err.Error())
			return
		}
		log.Println("读取ssl连接请求数据失败：" + err.Error())
		return
	}
	...
	i.response, err = i.Transport(i.request)
	if err != nil {
		log.Println("远程服务器响应失败：" + err.Error())
		return
	}
	if i.response == nil {
		log.Println("远程服务器无响应-2")
		return
	}
	defer func() {
		if i.response.Body != nil {
	...
	if err != nil {
		if strings.Contains(err.Error(), "aborted") {
			log.Println("代理返回响应数据失败：连接已关闭")
			return
		}
		log.Println("代理返回响应数据失败：" + err.Error())
	}
}
...
func (i *ProxyHttp) handleWsShakehandErr(rawProtolInput []byte) {
	...
	for _, value := range rawInputList {
		...
		if headerKeValList[0] == "Host" {
			...
			if err != nil {
				log.Println("解析ws请求地址错误：" + err.Error())
				return
			}
		}
		// 填充content-length
...
func (i *ProxyHttp) handleWsRequest() bool {
	...
	if err != nil {
		log.Println("升级ws协议失败：" + err.Error())
		return true
	}
	defer func() {
	...
	if err != nil {
		log.Println("连接ws服务器失败：" + err.Error())
		return true
	}
	defer func() {
