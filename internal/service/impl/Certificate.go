...
func (i *Certificate) GeneratePem(host string) ([]byte, []byte, error) {
	...
	template := x509.Certificate{
		...
		Subject: pkix.Name{ // Name代表一个X.509识别名。只包含识别名的公共属性，额外的属性被忽略。
			...
			Organization:       []string{"Vernom"},  // 证书存放的公司名称
			OrganizationalUnit: []string{"Vernom"},  // 证书所属的部门名称
			Province:           []string{"Klyntar"}, // 证书签发机构所在省
			CommonName:         host,
			Locality:           []string{"Klyntar"}, // 证书签发机构所在市
		},
		NotBefore:             time.Now().AddDate(-1, 0, 0),
		NotAfter:              time.Now().AddDate(1, 0, 0),
		KeyUsage:              x509.KeyUsageKeyEncipherment | x509.KeyUsageDigitalSignature | x509.KeyUsageCertSign,
		ExtKeyUsage:           []x509.ExtKeyUsage{x509.ExtKeyUsageServerAuth},
		EmailAddresses:        []string{"vernom@gmail.com"},
